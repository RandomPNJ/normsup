...
export default {
    ...
    services: [''],
    handler: (req, res, app) => monthlyConformity(req, res, app.get('AdminRegistry')),
};

export function monthlyConformity(req, res, AdminRegistry) {
    const user = req.body;
    ...
    return AdminRegistry.login(user.username, user.password)
        .then(result => {
            ...
            if(result) {
                return AdminRegistry.monthlyConformity()
                    .then(secondRes => {
                        return secondRes;
                    })
                    .catch(err => {
        ...
        .catch(err => {
            loggerT.verbose('monthlyConformity err', err);
        })
    ;
