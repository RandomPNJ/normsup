...
export default {
    ...
    uriPattern: '',
    services: [''],
    handler: (req, res, app) => downloadDocument(req, res, app.get('DocumentsRegistry')),
};

export function downloadDocument(req, res, DocumentsRegistry) {
    if(!req.decoded) {
    ...
    return DocumentsRegistry.downloadDocument(req.decoded, req.params.id)
        .then(result => {
            if(result) {
