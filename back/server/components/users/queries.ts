...
export const QUERY_GET_USERS = 'SELECT u.id,u.u.create_time,u.createdBy,u.email,u.lastname,u.name,u.organisation,u.username,u.address,u.phonenumber,u.postalCode,u.city, picture_url,ur.roleID,r.name AS rolename, c.org_name as companyName FROM `user` AS u LEFT JOIN `user_roles` AS ur ON u.id = ur.userID LEFT JOIN `roles` AS r ON ur.roleID = r.id  LEFT JOIN client AS c ON u.organisation = c.id WHERE u.organisation = ?';
