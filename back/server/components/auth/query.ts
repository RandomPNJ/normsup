...
export const CHECK_SUPPLIER_EMAIL_EXIST = 'SELECT * FROM suppliers as s INNER JOIN `account_activation` as aa ON s.id=aa.user_id WHERE s.email = ?';
...
export const CHECK_ACTIVATION = 'SELECT * from `account_activation` as aa INNER JOIN `suppliers` as s ON aa.user_id=s.id WHERE aa.token = ? LIMIT 1';
...
export const CHECK_RESET_PWD_TOKEN_SUPPLIER='SELECT COUNT(*) as count, user_id FROM `reset_password` as rp WHERE rp.type = \'SUPPLIER\' AND rp.token = ? AND rp.date IS NULL ';

export const CHANGE_PASSWORD='UPDATE `user` as u SET u.password = ? WHERE id = ?';
export const CHANGE_PASSWORD_SUPPLIER='UPDATE `suppliers` as s SET s.password = ? WHERE id = ?';
...
export const CHANGE_SUPPLIER_CREDS = 'UPDATE `suppliers` as s SET password = ? WHERE id = ?';