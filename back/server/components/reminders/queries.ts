...
export const DAILY_REMINDERS = 'SELECT o.denomination, r.client_id, r.email, gr.last_reminder, gr.frequency, gm.group_id, o.id FROM group_reminders as gr INNER JOIN group_members as gm ON gm.group_id = gr.group_id INNER JOIN organisations as o ON o.id = gm.member_id LEFT JOIN representatives as r ON o.id = r.organisation_id WHERE DATE_ADD(gr.last_reminder, INTERVAL gr.frequency DAY) <= ?;';

export const UPDATE_REMINDERS = 'UPDATE normsup.group_reminders as gr SET gr.last_reminder = ? WHERE ';
