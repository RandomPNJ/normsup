...
export default (expressApp, app, models, localOptions = {}) => {
  // expressApp => express object
  // app => service manager
  // models => model object
  // options => apiRoot = 'baseDir + /api', logger : loggerIO, showStack: false if prod


  // Check that schema are respected
  Joi.assert(localOptions, OptionsSchema);

  // Apply default option to localOptions, but no override
