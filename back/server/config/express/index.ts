...
export default (expressApp) => {
  ...
  const allowedOrigins = ['http://localhost:4200', 'https://app.normsup.com'];
  ...
  expressApp.use('/api', (req, res, next) => {
    ...
    if(req.url === '/auth/login' || req.url === '/auth/refresh_token' || req.url === '/auth/supplier-login' || req.url === '/auth/reset_password/modify' || req.url === '/auth/supplier_reset_password'
    ...
    if(req.url === '/supplier/currentSupplier' || req.url === '/documents/upload' || req.url.indexOf('/supplier/document_list') !== -1 
      || req.url.indexOf('/supplier/client_list') !== -1 || req.url.indexOf('/supplier/document/') !== -1) {
